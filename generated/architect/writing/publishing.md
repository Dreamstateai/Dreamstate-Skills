...
A scheduled content-generation request may create or revise reviewable drafts on its cadence, but it must not publish them. Scheduling a drafting job is not durable authority for any future outward delivery. Each `content.delivery_publish` requires explicit runtime authority for that exact final revision and destination when it is about to execute. If no authorized operator is present, leave the article saved, reviewed, or scheduled as a draft and report that publication remains blocked.
