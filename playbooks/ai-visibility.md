...
domain: seo
tier: playbook
tools_used: [visibility_overview, visibility_citations, visibility_ai_traffic, visibility_refresh, content_create_blog, content_generate_blog, content_get_blog, content_publish_blog]
---
...
Pull the headline numbers with `visibility_overview`: visibility score, citation coverage,
...
`visibility_refresh` (fire-and-forget; optionally scope to specific `query_ids`). It
...
Also read `visibility_ai_traffic` for GA4 AI-referral sessions (ChatGPT, Perplexity, etc.),
...
1. `content_create_blog` with a `title` and `content_type`. Optionally pass a `brief` that
