...
capability_ids: [visibility.overview, visibility.citations, visibility.ai_traffic, visibility.refresh, content.article_create_schedule, content.artifact_generate, content.article_get, content.article_delivery_create, content.delivery_publish]
---
...
1. `content.article_create_schedule` with a `title` and `content_type`. Optionally pass a `brief` that
   names the gap, the target query, and the angle, so the generation is on-point.
2. `content.artifact_generate` (`mode: "full"`). This is ASYNC: the item moves
   `draft -> researching -> ... -> draft_ready`. The response only confirms the job queued.
3. Poll `content.article_get` until `status` is `draft_ready`. Show the draft to the user.
4. On approval, `content.delivery_publish` (`self_hosted: true` to publish to the public
