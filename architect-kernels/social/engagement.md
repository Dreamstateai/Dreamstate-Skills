...
The LinkedIn comment-publish capability's target is a post URN, not a numeric id or a URL: pass the post's identity as `urn:li:activity:NUMERIC`, taken from wherever the post was surfaced (feed read, notification, engagement record), never constructed by hand from a display URL. If a mention is included in the reply, its `profile_id` is a person URN in the `urn:li:person:...` form this capability accepts, distinct from the `urn:li:member:...` form some other LinkedIn surfaces in this package require for an authored post's own mentions; do not assume one mention URN shape works everywhere a mention field appears, and never invent or guess a URN when the real one was not returned by a prior read.
